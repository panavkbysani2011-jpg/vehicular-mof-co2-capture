...
**Authors:** Panav and Advik Harihar
**Mentor:** Yesh Verma (Chemistry)
...
*[Figure 3.1: Methodology flowchart. Insert your Canva design here.]*
...
$$n_{Ca(OH)_2} = \dfrac{w\,\rho\,V}{M_{Ca(OH)_2}}$$
...
$$n_{remaining}(t) = n_{initial} - n_{CO_2}(t)$$
...
$$q_{eq} = m_{MOF}\,Q_{max}\,\dfrac{C_{CO_2}}{C_{ref}}$$
...
$$\sigma(T) = \dfrac{1}{1 + e^{-k_\sigma (T - T_m)}}$$
...
$$\sigma(T) = \dfrac{1}{1 + e^{(\Delta H - T\Delta S)/RT}}$$
...
$$k_\sigma = \dfrac{\Delta H}{R\,T_m^{2}}$$
...
$$Q_{total} = m_{MOF}\,c_{MOF}\,\Delta T + n_{CO_2}\,\Delta H_{des}$$
...
$$m_{CaCO_3} = n_{CO_2,released}\,M_{CaCO_3}$$
...
![Figure 3.4.1. Integrated capture unit (system schematic, Rhino build reference).](Figure_3.4.1_system_schematic.png){width=6.3in}
...
![Figure 3.4.2. Simulation algorithm (logic only).](Figure_3.4.2_algorithm_flowchart.png){width=4.8in}
...
![Figure 4.1. CO2 loading of the MOF bed over the cycle.](Figure_4.1_adsorption_curve.png){width=6in}
...
![Figure 4.2. Multi-cycle simulation, 85 C versus 110 C over ten cycles.](Figure_4.2_multicycle_results.png){width=6.3in}
...
The authors express sincere gratitude to our Chemistry Mentor, Yesh Verma, and the faculty at 10x International School, Mysore, for guidance, technical reviews, and mentorship throughout the development of this project. The authors also acknowledge the Scientific Review Committee of the IRIS National Fair for valuable preliminary feedback. This study builds on the published research of many groups working on metal-organic frameworks and carbon capture, whose work is cited throughout the paper.

## Declarations

**Originality.** The concept, system design, and simulation model presented in this paper are the authors' own work. The project was researched, designed, and developed by the authors under the guidance of their mentor. All external data and findings are taken from the published literature and are cited accordingly.

**Authors' Contributions.** The conceptual system architecture, mathematical derivations, numerical modeling, Python simulation scripts, and manuscript preparation were executed collaboratively by Panav Harihar and Advik Harihar under the mentorship of Yesh Verma.

**Data and code availability.** The complete simulation model, CAD technical schematics, and analytical scripts are openly available in the accompanying project repository for independent verification. All parameters and their sources are listed in the paper.
