...
[![Status: Submitted to NHSJS](https://img.shields.io/badge/Journal-NHSJS_Review-purple.svg)](https://nhsjs.com/)
...
**Paper Submission**: *National High School Journal of Science (NHSJS)*  
**Conference**: IRIS National Science Fair  

---

## Executive Summary

Conventional vehicular carbon capture concepts rely on heavy compressors and high-pressure gas cylinders (200 to 350 bar), introducing prohibitive energy penalties and safety hazards. This study proposes and simulates an unpressurized, passive capture-and-mineralization architecture that permanently sequesters tailpipe carbon dioxide directly into solid calcium carbonate (CaCO3, calcite).

### Core Findings (77-Minute Driving Cycle):
...
- **Passive Thermal Regeneration**: The 1063 kJ regeneration duty is passively supplied by engine exhaust waste heat in under 55 seconds without alternator or battery power.
- **Permanent Solid Mineralization**: Desorbed gas reacts irreversibly with a 30 L limewater slurry (20 wt% Ca(OH)2), precipitating 0.735 kg (at 85 deg C) to 1.678 kg (at 110 deg C) of high-purity solid calcite.
- **Chassis Integration Feasibility**: Sorbent bed envelope occupies 5.56 L, induces an exhaust back-pressure of only 0.68 kPa (automotive limit: 5.0 kPa), and imposes a modest 3.07% fuel economy penalty on a 1500 kg passenger car.

---

## System Architecture

The onboard system operates in three synchronized stages:
...
2. **Coaxial Waste Heat Exchanger**: When bed reaches target saturation, a bypass diverts high-temperature exhaust gas (300 to 600 deg C) around the bed jacket, thermally driving off CO2 at 85 to 110 deg C.
...
Detailed derivations, nomenclature, and governing equations are documented in [`docs/ALGORITHM_AND_MODEL.md`](docs/ALGORITHM_AND_MODEL.md).
...
- **Six Publication Charts**:
  - Adsorbed Mass vs. Time (q(t) breakthrough)
  - Calcite Produced vs. Time (CaCO3 precipitation)
...
- **Packaging and Back-Pressure Gauge**: Evaluates cylinder dimensions (D x L), back-pressure (<5.0 kPa safe limit check), and vehicle weight penalty.
...
The simulator is 100% static with zero dependencies, making it instantly deployable on Vercel:
...
If you use this model, simulation code, or conceptual architecture in your research, please cite:

```bibtex
@article{harihar2026onboard,
